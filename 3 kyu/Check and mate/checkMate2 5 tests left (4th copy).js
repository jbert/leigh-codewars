  ...
  function takeOrBlockOrEnPassant() {
    ...
    function playerTakesThreat() {
      ...
      (function responseMoveTakePiece() {
        ...
        return (responsePieceRangeArr);
      }());
      console.log('responsePieceRangeArr');
      console.log(responsePieceRangeArr);
      console.log(board.checkPieces);

      // just removing from board.checkPieces the piece blocked and putting marker on the blocker, so can't be moved to take another piece
    ...
    (function buildAttackVectors() {
      ...
      // can any other pieces reply moves take the remaining piece in board.checkPieces? (playerTakesThreat)
      // can any in replyPiecesArray move into a block square if array contains only straight line pieces. if it's a knight and can't take then it is mate


    /* 1) check each piece in board.checkPieces, check the vector from the threat piece to
    the king, is that being blocked currenty by own pieces? Save any others in a new array (notCurrentlyBlocked array)
       2) can that piece in new array be taken by current own pieces? (could perform a check that if you remove the piece and take that threat piece it isn't blocking threat vectors - save that piece from previous in a 'threatBlocker' array - so piece cannot move)
    */
